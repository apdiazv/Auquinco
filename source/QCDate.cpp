...
//	Constructor for date of a given day, month and year
...
// Friend style function to add nStep to a date
QCDate getShift(const QCDate& date1, int nStep)
{
	if(nStep == 0)
	{
		return getBussDay(date1);
	}
	else
    {
		QCDate date(date1.excelSerialDate());
		for(int i = 1; i <= nStep; i++)
		{
			date = getBussDay(date.addDays(1));
		}
		return date;
    }
}

// Friend style function to subtract nStep to a date
QCDate getLag(const QCDate& date1, int nStep)
{
	if(nStep == 0)
	{
		return getPrevDay(date1);
	}
	else
    {
		QCDate date(date1.excelSerialDate());
		for(int i = 1; i <= nStep; i++)
		{
			date = getPrevDay(date.addDays(-1));
		}
		return date;
    }
}
